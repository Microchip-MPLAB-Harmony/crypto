...
static uint32_t TC1_CH0_TimerStatus;  /* saves interrupt status */

/* Callback object for channel 0 */
TC_CALLBACK_OBJECT TC1_CH0_CallbackObj;
...
bool TC1_CH0_TimerPeriodHasExpired(void)
{
    ...
    timer_status = ((TC1_CH0_TimerStatus | TC1_REGS->TC_CHANNEL[0].TC_SR) & TC_SR_CPCS_Msk) >> TC_SR_CPCS_Pos;
    TC1_CH0_TimerStatus = 0U;
    NVIC_EnableIRQ(TC1_CH0_IRQn);
    return timer_status;
}

/* Register callback for period interrupt */
void TC1_CH0_TimerCallbackRegister(TC_CALLBACK callback, uintptr_t context)
{
