 ...
 *************************************************************/
static CONST cryptoST_testVector_t satcZ =
{
    .name = DATA_PACKAGE_NAME "_NULL",
    .source = sha_all,
    .description = sha_all_description,
...
static __attribute__((unused)) CONST cryptoST_testDetail_t test_item[] =
{
    ...
    },
    {
        ...
        .goldenCipher = { // no data defined here -- get it later
            .length = 32,
            .data = (uint8_t*)(uint8_t[]) // reserve room for the golden hash's
...
static char * openData_func(void)
{
    ...
    if (satcZ.vector.data)
    {
        memset(satcZ.vector.data,0,ZERO_MAX);
        return NULL;
    }
    else return 
...
{
    if (satcZ.vector.data) 
        cryptoSTE_free(satcZ.vector.data);
